...
export class App {
  ...
  loadState() {
    this.pages = JSON.parse(
      localStorage.getItem("pages") ||
        JSON.stringify([{ name: "Page 1", content: "" }])
    );
  ...
  updateView() {
    ...
    this.elements.currentPageTitle.textContent =
      this.pages[this.currentPage].name;
  }
  ...
  setupEventListeners() {
    ...
    document
      .getElementById("context-menu-delete-button")
    ...
    document
      .getElementById("context-menu-rename-button")
      .addEventListener("click", () => {
